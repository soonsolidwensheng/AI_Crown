...
if __name__ == "__main__":
    ...
    #     break
    save_dir = 'test_data_/occlu_c6735656-818b-4570-8d01-36e476403d8e'
    case_id = 'post_ac4d398d-da28-4ac7-ac38-67b532dc95d2'
    
    with open(os.path.join(save_dir, case_id, "input.json"), "r") as f:
        event = json.load(f)
    ...
    # with open(os.path.join(save_dir, case_id, "std.json"), "r") as f:
    #     event_s = json.load(f)
    # event['standard'] = event_['standard']
    # event_['cpu_std_json']['cpu_colors_info'] = event_s['cpu_std_json']['cpu_colors_info']
    event['cpu_std_json'] = event_['cpu_std_json']
    # event['paras'] = event['params']
    # event['paras']['morph_template'] = 'st_tooth'
    event['paras'] = {}
    event['paras']['adjust_crown'] = 0
    out = handler(event, os.path.join(save_dir, case_id))
    with open(os.path.join(save_dir, case_id, 'post.json'), "w") as f:
        f.write(json.dumps(out["Msg"]["data"]))
    print